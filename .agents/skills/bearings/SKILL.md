...
Every `/bearings` chat response renders EXACTLY these four sections, in THIS order, and nothing else structural (there is no At Anchor section); their default headings follow [`AGENTS.md`'s presentation preferences](../../../AGENTS.md#presentation-preferences) for display role wording only, never grouping or order:
...
- Apply [`AGENTS.md`'s presentation preferences](../../../AGENTS.md#presentation-preferences) to direct address; when required, include it inside one item or empty-state sentence.
