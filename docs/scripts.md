...
| `fm-enroll-herdr.sh`     | Enroll an existing agent-free Herdr endpoint under the [retained enrollment contract](herdr-backend.md#retained-endpoint-enrollment) |
| `fm-pi-session-check.sh` | Validate exact stopped Pi history before retained enrollment or recovery |
...
| `fm-control.sh`          | Drive the [agent lifecycle and Herdr endpoint control plane](agent-control.md) for an exact task id |
